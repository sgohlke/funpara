import type { DateFunction, ExitFunction, FetchFunction, TimeoutFunction } from '@/index'
import {
...
class Logger {
   ...
   */
  prepareLogMessage(message: string, dateFunction: DateFunction = nowDateFunction()): string {
...
async function getUserById(
  ...
  try {
    const userDataResponse = await fetchFunction(`https://localhost:3000/users/${id}`)
...
async function getJSONMessage(
  ...
  try {
    const messageResponse = await fetchFunction('https://localhost:3000/message/')
    if (!messageResponse.ok) {
      return { data: { message: 'Message error!' } }
    }

    if (messageResponse.headers.get('Content-Type') !== 'application/json') {
      return {
        data: {
...
test('Test date functions', () => {
  ...
  assert.deepStrictEqual(
    logger.prepareLogMessage('I am a log message!', fixedDateFunction('2023-09-06T00:00:00Z')),
  ...
  assert.ok(logger.prepareLogMessage('I am a log message!').includes('I am a log message!'))
})

test('Test fetch functions', async () => {
  // Test that fixedResponseFetchFunction return the correct response
  assert.deepStrictEqual(
    await getUserById('1', fixedResponseFetchFunction('John Doe', { status: 200 })),
    'User John Doe',
  )
  ...
  assert.deepStrictEqual(
    await getJSONMessage(
      fixedResponseFetchFunction('{"data": {"message": "Hello world!"}}', {
  ...
  assert.deepStrictEqual(await getJSONMessage(unknownContentTypeFetchFunction), {
...
test('Test timeout functions', () => {
  ...
  assert.deepStrictEqual(
    noCallbackTimeoutFunction(() => logger.log('This should not be executed!'), 1000),
    1,
  )
